...
void FraudTwoTowerTest::testingWithRealData(int numDataSplits, int dataBatchSize, int numRows, int numCols, std::string orderFilePath, std::string modelFilePath) {
    ...
    auto myPlan2 = exec::test::PlanBuilder(planNodeIdGenerator, pool_.get())
                         ...
                         .project({"p_product_id", "c_customer_sk", "avg_customer_rating", "p_dept", "avg_product_rating", "concat(embedding_customer(convert_int_array(c_customer_sk)), embedding_addr(convert_int_array(c_address_num)), embedding_age(convert_int_array(age)), embedding_country(convert_int_array(c_birth_country)), get_customer_extra_feature(c_cust_flag, CAST(avg_customer_rating AS REAL))) as customer_feature"})
                         .project({"p_product_id", "c_customer_sk", "avg_customer_rating", "customer_feature", "concat(embedding_product(convert_int_array(p_product_id)), embedding_dept(convert_int_array(p_dept)), get_product_rating(CAST(avg_product_rating AS REAL))) as product_feature"})
                         .project({"p_product_id", "c_customer_sk", "avg_customer_rating", "product_feature", "relu(batch_norm3_customer(mat_vector_add_3_customer(mat_mul_3_customer(relu(batch_norm2_customer(mat_vector_add_2_customer(mat_mul_2_customer(relu(batch_norm1_customer(mat_vector_add_1_customer(mat_mul_1_customer(customer_feature)))))))))))) AS customer_encoding"})
                         ...
                         .project({"p_product_id", "c_customer_sk", "avg_customer_rating", "vector_addition(customer_encoding, product_encoding) as final_encoding"})
                         .filter("avg_customer_rating >= 4.0")
                         .planNode();
